...
function parseFormatString(format) {
  ...
  return knownFormats[format] = {
    parser(dateStr, locale) {
      // return dayjs(dateStr).tz(locale)
      return parseDateFns(dateStr, format, new Date())
...
console.log('dateParts', dateStr, locale, dateParts)
      ...
      const res = partParserKeys.reduce((origDate, key) => {
...
export function parseDate(dateStr, format, locale, timeArr = [0, 0, 0, 0]) {
  if (dateStr instanceof Date || typeof dateStr === 'number') {
    const date = stripTime(dateStr, timeArr);
