...
function processInputDates(datepicker, inputDates, clear = false) {
  ...
  const {config, dates: origDates, rangeSideIndex, times: origTimes} = datepicker;
  const { hour, minute, second } = origTimes || {hour: 0, minute: 0, second: 0};
  const {pickLevel, maxNumberOfDates} = config;
  let newDates = inputDates.reduce((dates, dt) => {
    let date = parseDate(dt, config.format, config.locale, [hour, minute, second, 0]);
    if (date === undefined) {
      return dates;
    }
...
function refreshUI(datepicker, mode = 3, quickRender = true, viewDate = undefined, timeViewUpdate = '', selectedValue = '') {
  const {config, picker, inputField} = datepicker;

  // if select time, prevent changing view
  if (mode & 2) {
    const newView = picker.active ? config.pickLevel : config.startView;
    picker.update(viewDate, selectedValue)
    !timeViewUpdate && picker.changeView(newView)
    picker.render(quickRender, timeViewUpdate);
  }
  // if (isSelectTime) {
  //   picker
  // }
...
function setDate(datepicker, inputDates, options, inputTimes, isMinView = true) {
  ...
  } else if (timeViewUpdate) {
    datepicker.times = newTimes;
    refreshUI(datepicker, render ? 3 : 1, true, viewDate, timeViewUpdate);
  } else {
    refreshUI(datepicker, forceRefresh ? 3 : 1, true, viewDate, timeViewUpdate);
  }
