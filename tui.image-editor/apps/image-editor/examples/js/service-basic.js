...
$selectLine.on('change', function () {
  ...
  if (mode === 'freeDrawing') {
    ...
    imageEditor.startDrawingMode('LINE_DRAWING', {...settings, arrowType: {head: 'triangle', tail: 'chevron'}});
    console.log('STARTED LINE', imageEditor.getGraphic(), imageEditor.getCanvas())
    ...
    if (canvas) {
      canvas.on('mouse:over', function(e) {
        ...
        // if (!e.target  || activeObject === e.target) return;
        // curObj = e.target;
        // e.target.set('fill', '#ddd');  
        // e.target.set('strokeDashArray', [5]) 
        // e.target.set('stroke', 'red')
        // btn.style.display = 'block'
        // positionBtn(e.target)
        // closeContainer.style.border = '1px dashed red'
